...
private readonly HOME_BEST_UPDATE_KEY = 'axe_os_last_best_update';
  ...
  constructor(
    ...
    // Live-Ticker fÃÂÃÂÃÂÃÂ¼r den Timer (aktualisiert die Anzeige sekÃÂÃÂÃÂÃÂ¼ndlich)
...
this.ngZone.runOutsideAngular(() => {
  ...
  public toggleWidgetVisibility(id: string): void {
    if (this.hiddenWidgets.has(id)) {
      // Show widget ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ restore stashed DOM element
      ...
      // Hide widget ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ remove from grid and stash the DOM element
...
// --- FINALER, FELSENFESTER TIMER (FIRMWARE-GESTÃÂÃÂÃÂÃÂTZT) ---
    ...
    // Datum setzen, ab dem der Timer zÃÂÃÂÃÂÃÂ¤hlt
    this.lastBestUpdate = new Date(Date.now() - elapsedMs);

    // Display-Info fÃÂÃÂÃÂÃÂ¼r das UI aktualisieren
    this.displayInfo = { 
        ...info, 
    ...
    // --- ENDE ---
        ...
        // BERECHNUNG MIT GEGLÃÂÃÂÃÂÃÂTTETEM FILTER (DÃÂÃÂÃÂÃÂMPFUNG)
        if (info && info.power > 0 && info.hashRate_1h > 0) {
          ...
          if (this.smoothedEfficiencyAverage === 0) {
            ...
            // Das schluckt jede sekÃÂÃÂÃÂÃÂ¼ndliche Schwankung komplett!
            this.smoothedEfficiencyAverage = (this.smoothedEfficiencyAverage * 0.99) + (currentRawAvg * 0.01);
          }

          // Weise den geglÃÂÃÂÃÂÃÂ¤tteten Wert der Anzeige zu
        ...
        // MANUELLE BERECHNUNG FÃÂÃÂÃÂÃÂR DEN GEGLÃÂÃÂÃÂÃÂTTETEN DURCHSCHNITT:
        if (info && info.power > 0 && info.hashRate_1h > 0) {
            const currentRawAvg = (info.power * 1000) / info.hashRate_1h;
            ...
            if (this.smoothedEfficiencyAverage === 0) {
                ...
                // FILTER: 99% alter Wert + 1% neuer Wert (schluckt sekÃÂÃÂÃÂÃÂ¼ndliche Schwankungen)
                this.smoothedEfficiencyAverage = (this.smoothedEfficiencyAverage * 0.99) + (currentRawAvg * 0.01);
            }
  ...
  private setTitle(info: ISystemInfo, systemInfoError: ISystemInfoError) {
    ...
    if (info.showNewBlock) {
      parts.push('Block found ÃÂÃÂ°ÃÂÃÂÃÂÃÂÃÂÃÂ');
      ...
      parts.push(
        ...
        (info.temp ? `${info.temp}${info.temp2 > -1 ? `/${info.temp2}` : ''}${info.vrTemp ? `/${info.vrTemp}` : ''} ÃÂÃÂÃÂÃÂ°C` : ''),
        (!info.power_fault ? `${info.power} W` : ''),
        (info.bestDiff ? DiffSuffixPipe.transform(info.bestDiff) : ''),
      );
    }

    this.titleService.setTitle(parts.filter(Boolean).join(' ÃÂÃÂ¢ÃÂÃÂÃÂÃÂ¢ '));
  }
  ...
  static getSettingsForLabel(label: eChartLabel): { suffix: string; precision: number } {
    switch (label) {
      ...
      case eChartLabel.vrTemp: return { suffix: ' ÃÂÃÂÃÂÃÂ°C', precision: 1 };
      case eChartLabel.asicVoltage:
      case eChartLabel.voltage:
