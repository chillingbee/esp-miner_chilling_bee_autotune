...
typedef struct
{
    ...
    uint16_t TPS546_INIT_FREQUENCY; 
...
// 110 -> After shutting down, wait one HICCUP period, and attempt to restart up to 6 times. After 6 failed restart attempts, do not
// attempt to restart (latch off). 111 -> Shutdown delay of seven PWM_CLK, HICCUP equal to 7 times TON_RISE
...
// 111 -> After shutting down, wait one HICCUP period, and attempt to restart indefinitely, until commanded OFF or a successful
// start-up occurs. 111 -> Shutdown delay of 7 ms, HICCUP equal to 4 times TON_RISE
...
#define ON_OFF_CONFIG_CMD                                                                                                          \
    0x08 // Act on (01h) OPERATION Command (and CONTROL pin if configured by CP) to start/stop power conversion.
#define ON_OFF_CONFIG_CP                                                                                                           \
    0x04 // Act on CONTROL pin (and (01h) OPERATION Command if configured by bit [3]) to start/stop power conversion.
#define ON_OFF_CONFIG_POLARITY 0x02 // CONTROL pin has active high polarity.
#define ON_OFF_CONFIG_DELAY                                                                                                        \
    0x01 // When power conversion is commanded OFF by the CONTROL pin (must be configured to respect the CONTROL pin as above), stop
